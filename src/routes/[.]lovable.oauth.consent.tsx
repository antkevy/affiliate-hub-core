...
export const Route = createFileRoute("/.lovable/oauth/consent")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Autorize uma integração de agente na sua conta Affiliate Hub.",
      },
  ...
  loader: async ({ deps }) => {
    const { data, error } = await supabase.auth.oauth.getAuthorizationDetails(deps.authorizationId);
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 py-10">
      <section className="panel w-full max-w-md p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          O agente poderá consultar seu painel, ofertas, links e automações. Seus dados continuam
          protegidos pela sua conta.
        </p>
